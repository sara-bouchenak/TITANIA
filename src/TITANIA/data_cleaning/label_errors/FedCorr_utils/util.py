...
def add_noise(args, y_train, dict_users):
    ...
    for i in np.where(gamma_c > 0)[0]:
        ...
        y_train_noisy[sample_idx[noisy_idx]] = np.random.randint(0, 2, len(noisy_idx))
...
def lid_term(X, batch, k=20):
    ...
    at=np.apply_along_axis(np.argsort, axis=1, arr=distances)
    
    # get the closest k neighbours
    sort_indices = np.apply_along_axis(np.argsort, axis=1, arr=distances)[:, 1:k + 1]
    m, n = sort_indices.shape #was this change right
    idx = np.ogrid[:m, :n]
    idx = list(idx)
    idx[1] = sort_indices
    # sorted matrix
