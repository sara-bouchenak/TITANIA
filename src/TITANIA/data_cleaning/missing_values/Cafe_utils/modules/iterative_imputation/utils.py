...
def fit_one_feature(X_filled, y, missing_mask, col_idx, estimator, num_cols, compute_proj=False, regression = False):
	...
	row_mask = missing_mask.iloc[:, col_idx]
	X_train = X_filled[~row_mask].iloc[:, np.arange(X_filled.shape[1]) != col_idx]
	y_train = X_filled[~row_mask].iloc[:, col_idx]
	...
	if not regression:
		...
		y_oh = oh.fit_transform(y.values.reshape(-1, 1)).toarray()
	...
	y_ = row_mask
	if row_mask.sum() == 0:
		coef = np.zeros(X_.shape[1]+1) + 0.001 #could put +1
		"""
		if len(coef)!=103:
			print("here2",len(coef))
		"""
		lr = None
	else:
        ...
        if len(coef)!=104:
			...
			print(len(lr.coef_[0]),len(lr.intercept_))
        """
...
def impute_one_feature(X_filled, missing_mask, col_idx, estimator, num_cols, min_value=None, max_value=None):
	...
	X_test = X_filled[row_mask].iloc[:, np.arange(X_filled.shape[1]) != col_idx]
	...
	X_filled.iloc[row_mask, col_idx] = np.squeeze(imputed_values)
